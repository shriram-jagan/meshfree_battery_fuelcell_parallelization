...
def compute_phi_M_int(x_nodes_interface, x_nodes, a, M, M_P_x, M_P_y):
    ...
    for i in range(np.shape(x_nodes_interface)[0]):
        ...
        for j in range(np.shape(x_nodes)[0]):
            ...
            z_ij = (
                (
                    (x_nodes_interface[i, 0] - x_nodes[j, 0]) ** 2
                    + (x_nodes_interface[i, 1] - x_nodes[j, 1]) ** 2
                )
                ** 0.5
            ) / a[j]
            z_ij_P_x = (x_nodes_interface[i, 0] - x_nodes[j, 0]) / (
                a[j] * z_ij * a[j] + 2.220446049250313e-16
            )  # partial z partial x, add the small number to force the term with machine accuracy
            z_ij_P_y = (x_nodes_interface[i, 1] - x_nodes[j, 1]) / (
                a[j] * z_ij * a[j] + 2.220446049250313e-16
            )  # partial z partial y
            ...
            H_T = np.array(
                [
                    1,
                    (x_nodes_interface[i][0] - x_I[0]),
                    (x_nodes_interface[i][1] - x_I[1]),
                ],
                dtype=np.float64,
            )
            H = np.transpose(H_T)
...
def shape_grad_shape_func_int(
    ...
    for ii in range(num_non_zero_phi_a):
        ...
        H_T = np.array(
            [1, (x_nodes_interface[i][0] - x_I[0]), (x_nodes_interface[i][1] - x_I[1])],
            dtype=np.float64,
        )
        H = np.transpose(H_T)
