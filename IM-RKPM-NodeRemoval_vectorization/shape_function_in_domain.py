...
def compute_phi_M(
    ...
    for i in range(np.shape(x_G)[0]):
        ...
        if i not in saved_dist_func_index:
            ...
            save_point_D_coor.append(
                [x_coor_min_point_segment, y_coor_min_point_segment]
            )
            save_distance_function.append([i, x_G[i, 0], x_G[i, 1], min_distance])
            save_distance_function_dx.append(
                [
                    i,
                    x_G[i, 0],
                    x_G[i, 1],
                    (x_G[i, 0] - x_coor_min_point_segment) / min_distance,
                ]
            )
            save_distance_function_dy.append(
                [
                    i,
                    x_G[i, 0],
                    x_G[i, 1],
                    (x_G[i, 1] - y_coor_min_point_segment) / min_distance,
                ]
            )
