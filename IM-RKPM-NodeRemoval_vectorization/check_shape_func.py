...
def allign_G_nodes(
    ...
    for i in range(num_G):
        for j in range(num_G):
            if (
                abs(x_G[i, 0] - x_G_kristen[j, 0]) < 1e-13
                and abs(x_G[i, 1] - x_G_kristen[j, 1]) < 1e-13
            ):
...
# plt.scatter(x_G[:, 0], x_G[:, 1], c=np.linalg.norm(x_G-np.asarray(alligned_x_G_kristen),2,axis=1))
plt.scatter(
    x_G[np.array(mismatched_index_grad_shape_x)[:, 0], 0],
    x_G[np.array(mismatched_index_grad_shape_x)[:, 0], 1],
    c=mismatch_grad_x,
)
plt.colorbar()
plt.title("log(difference of x_G)")
...
plt.scatter(
    x_nodes[np.array(mismatched_index_grad_shape_y)[:, 1], 0],
    x_nodes[np.array(mismatched_index_grad_shape_y)[:, 1], 1],
    c=mismatch_grad_y,
)
plt.colorbar()
