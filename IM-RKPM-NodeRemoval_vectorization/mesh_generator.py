...
def set_length_scale(surface_id, lcmin, lcmax, dismin, distmax):
    ...
    gmsh.model.mesh.field.setNumber(threshold, "DistMin", dismin)
...
seal_length = (
    2.0 / 1000
)  # width of seal (distance from panel edge to frame, parameter f)
seal_thick = (
...
frame_extended = (
    0.1  # the frame is created longer than panel then the extended part is cutted off
)
...
gmsh.model.occ.translate(
    frame_surface_right, frame_extended + panel_length + seal_length, 0, 0
)
...
gmsh.model.occ.translate(
    frame_surface_rear, 0, frame_extended + panel_width + 2 * seal_length, 0
)
...
gmsh.model.occ.fragment(
    [(3, front_glass)], [(3, front_encap)], removeTool=True
)  # remove repeated interfaces, index of glass and front_enc are not changed
gmsh.model.occ.fragment(
    [(3, front_encap)], cell_eva_frag[0], removeTool=True
)  # remove repeated interfaces, all index of involved volumes are not changed
gmsh.model.occ.fragment(
    cell_eva_frag[0], [(3, back_encap)], removeTool=True
)  # remove repeated interfaces, all index of involved volumes are not changed
gmsh.model.occ.fragment(
    [(3, back_encap)], [(3, back_sheet)], removeTool=True
...
count_volumes, structure_vol_list = volume_tags(
    vol_list, count_volumes, structure_vol_list
)
...
threshold = set_length_scale(
    "back_sheet", back_sheet_thick * 10, back_sheet_thick * 100, 0, 0.1
)  # 3
min_dist.append(threshold)
threshold = set_length_scale(
    "back_encap", back_encap_thick * 10, back_encap_thick * 100, 0, 0.1
)  # 3
min_dist.append(threshold)
threshold = set_length_scale(
    "front_encap", front_encap_thick * 10, front_encap_thick * 100, 0, 0.1
)  # 3
min_dist.append(threshold)
threshold = set_length_scale(
    "front_glass", front_glass_thick * 10, front_glass_thick * 100, 0, 0.1
)  # 3
min_dist.append(threshold)
...
for i in range(n_cell_length):
    for j in range(n_cell_width):
        threshold = set_length_scale(
            "cell" + str(i * n_cell_width + j), cell_thick * 0.5, cell_thick * 1, 0, 0.1
        )  # 3
        min_dist.append(threshold)
