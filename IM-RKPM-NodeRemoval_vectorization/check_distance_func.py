...
for i in range(num_G):
    for j in range(num_G):
        if (
            abs(G_coor_Xin[i, 0] - G_coor_Kristen[j, 0]) < 1e-10
            and abs(G_coor_Xin[i, 1] - G_coor_Kristen[j, 1]) < 1e-10
        ):
...
max_diff_index = np.argmax(
    abs(np.asarray(distance_func_dy_Xin - distance_func_dy_Kristen_alligned))
)
...
plt.scatter(
    G_coor_Xin[:, 0],
    G_coor_Xin[:, 1],
    c=distance_func_Xin - distance_func_Kristen_alligned,
)
...
plt.scatter(
    G_coor_Xin[:, 0],
    G_coor_Xin[:, 1],
    c=distance_func_dy_Xin - distance_func_dy_Kristen_alligned,
)
...
plt.scatter(
    G_coor_Xin[:, 0],
    G_coor_Xin[:, 1],
    c=distance_func_dx_Xin - distance_func_dx_Kristen_alligned,
)
