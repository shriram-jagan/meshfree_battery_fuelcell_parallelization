...
single_grain = (
    "Faulse"  # True: single grain, Faulse: multiple grains read from an image
)
...
node_ID_to_plot_1_coor = np.array(
    [-4.400000000000000594e-06, -4.000000000000000666e-06]
)  # the coordinates of first node whose shape function will be plotted
node_ID_to_plot_2_coor = np.array(
    [-4.400000000000000594e-06, -4.400000000000000224e-06]
)  # the coordinates of second node whose shape function will be plotted
node_ID_to_plot_interface_coor = np.array(
    [-4.400000000000000594e-06, -4.200000000000000445e-06]
)  # the coordinates of node on interface whose shape function will be plotted

node_ID_to_plot_1_index = np.where(
    np.linalg.norm(x_nodes - node_ID_to_plot_1_coor, 2, axis=1) < 1e-10
)[0][0]
node_ID_to_plot_2_index = np.where(
    np.linalg.norm(x_nodes - node_ID_to_plot_2_coor, 2, axis=1) < 1e-10
)[0][0]
node_ID_to_plot_interface_index = np.where(
    np.linalg.norm(x_nodes - node_ID_to_plot_interface_coor, 2, axis=1) < 1e-10
)[0][0]
...
for i in range(grid_num + 1):
    for j in range(grid_num + 1):
        ...
        plotting_points_1_x = (
            node_ID_to_plot_1_coor[0]
            - node_ID_to_plot_1_a
            + 2 * node_ID_to_plot_1_a / grid_num * (i)
        )
        # x_1.append(plotting_points_1_x)
        plotting_points_1_y = (
            node_ID_to_plot_1_coor[1]
            - node_ID_to_plot_1_a
            + 2 * node_ID_to_plot_1_a / grid_num * (j)
        )
        # y_1.append(plotting_points_1_y)

        plotting_points_2_x = (
            node_ID_to_plot_2_coor[0]
            - node_ID_to_plot_2_a
            + 2 * node_ID_to_plot_2_a / grid_num * (i)
        )
        # x_2.append(plotting_points_2_x)
        plotting_points_2_y = (
            node_ID_to_plot_2_coor[1]
            - node_ID_to_plot_2_a
            + 2 * node_ID_to_plot_2_a / grid_num * (j)
        )
        # y_2.append(plotting_points_2_y)

        plotting_points_interface_x = (
            node_ID_to_plot_interface_coor[0]
            - node_ID_to_plot_interface_a
            + 2 * node_ID_to_plot_interface_a / grid_num * (i)
        )
        # x_inter.append(plotting_points_interface_x)
        plotting_points_interface_y = (
            node_ID_to_plot_interface_coor[1]
            - node_ID_to_plot_interface_a
            + 2 * node_ID_to_plot_interface_a / grid_num * (j)
        )
        # y_inter.append(plotting_points_interface_y)

        if (
            (plotting_points_1_x - node_ID_to_plot_1_coor[0]) ** 2
            + (plotting_points_1_y - node_ID_to_plot_1_coor[1]) ** 2
        ) ** 0.5 < node_ID_to_plot_1_a:
            ...
            for i_interface_seg in range(num_interface_segments):
                BP = np.array(
                    [
                        plotting_points_1_x - BxByCxCy[i_interface_seg, 0],
                        plotting_points_1_y - BxByCxCy[i_interface_seg, 1],
                    ]
                )
                BC = np.array(
                    [
                        BxByCxCy[i_interface_seg, 2] - BxByCxCy[i_interface_seg, 0],
                        BxByCxCy[i_interface_seg, 3] - BxByCxCy[i_interface_seg, 1],
                    ]
                )
                CP = np.array(
                    [
                        plotting_points_1_x - BxByCxCy[i_interface_seg, 2],
                        plotting_points_1_y - BxByCxCy[i_interface_seg, 3],
                    ]
                )
                if (
                    np.cross(BP, BC) == 0
                    and np.dot(BP, BC) >= 0
                    and np.dot(BP, BC) <= np.dot(BC, BC)
                ):
                    point_not_on_interface = "False"

            if point_not_on_interface == "True":

                node_1_plotting_points.append(
                    [plotting_points_1_x, plotting_points_1_y]
                )

                if plotting_points_1_y > node_ID_to_plot_interface_coor[1]:
                    node_1_plotting_points_grain_id.append(
                        nodes_grain_id[node_ID_to_plot_1_index]
                    )
                else:
                    node_1_plotting_points_grain_id.append(
                        nodes_grain_id[node_ID_to_plot_2_index]
                    )

                num_plot_points_node_1 += 1

        if (
            (plotting_points_2_x - node_ID_to_plot_2_coor[0]) ** 2
            + (plotting_points_2_y - node_ID_to_plot_2_coor[1]) ** 2
        ) ** 0.5 < node_ID_to_plot_2_a:
            point_not_on_interface = "True"

            for i_interface_seg in range(num_interface_segments):
                BP = np.array(
                    [
                        plotting_points_2_x - BxByCxCy[i_interface_seg, 0],
                        plotting_points_2_y - BxByCxCy[i_interface_seg, 1],
                    ]
                )
                BC = np.array(
                    [
                ...
                CP = np.array(
                    [
                        plotting_points_2_x - BxByCxCy[i_interface_seg, 2],
                        plotting_points_2_y - BxByCxCy[i_interface_seg, 3],
                    ]
                )
                if (
                    np.cross(BP, BC) == 0
            ...
            if point_not_on_interface == "True":
                node_2_plotting_points.append(
                    [plotting_points_2_x, plotting_points_2_y]
                )

                if plotting_points_2_y > node_ID_to_plot_interface_coor[1]:
                    node_2_plotting_points_grain_id.append(
                        nodes_grain_id[node_ID_to_plot_1_index]
                    )
                else:
                    node_2_plotting_points_grain_id.append(
        ...
        if (
            (plotting_points_interface_x - node_ID_to_plot_interface_coor[0]) ** 2
            + (plotting_points_interface_y - node_ID_to_plot_interface_coor[1]) ** 2
        ) ** 0.5 <= node_ID_to_plot_interface_a:
            point_not_on_interface = "True"
            ...
            for i_interface_seg in range(num_interface_segments):
                BP = np.array(
                    [
                        plotting_points_interface_x - BxByCxCy[i_interface_seg, 0],
                        plotting_points_interface_y - BxByCxCy[i_interface_seg, 1],
                    ]
                )
                BC = np.array(
                    [
                ...
                CP = np.array(
                    [
                        plotting_points_interface_x - BxByCxCy[i_interface_seg, 2],
                        plotting_points_interface_y - BxByCxCy[i_interface_seg, 3],
                    ]
                )
                if (
                    np.cross(BP, BC) == 0
            ...
            if point_not_on_interface == "True":
                ...
                node_interface_plotting_points.append(
                    [plotting_points_interface_x, plotting_points_interface_y]
                )
                if plotting_points_interface_y > node_ID_to_plot_interface_coor[1]:
                    node_interface_plotting_points_grain_id.append(
                        nodes_grain_id[node_ID_to_plot_1_index]
                    )
                else:
                    node_interface_plotting_points_grain_id.append(
...
all_plotting_points = np.concatenate(
    (
        plotting_points_coor_node_1,
        plotting_points_coor_node_2,
        plotting_points_coor_node_interface,
        plotting_points,
    ),
    axis=0,
)  # similarly to x_G

all_plotting_grain_id = np.concatenate(
    (
        np.array(node_1_plotting_points_grain_id),
        np.array(node_2_plotting_points_grain_id),
        np.array(node_interface_plotting_points_grain_id),
        plotting_grain_id,
    ),
    axis=0,
)
...
num_all_plotting_points = (
    num_plot_points_node_1 + num_plot_points_node_2 + num_plot_points_node_interface + 1
)
...
M_P_x = np.array(
    [np.zeros((3, 3)) for _ in range(num_all_plotting_points)]
)  # partial M partial x
M_P_y = np.array(
    [np.zeros((3, 3)) for _ in range(num_all_plotting_points)]
)  # partial M partial y
...
kernal_func_node_2 = kernal_func_array[
    num_plot_points_node_1 : (num_plot_points_node_1 + num_plot_points_node_2),
    node_ID_to_plot_2_index,
]
kernal_func_node_interface = kernal_func_array[
    (num_plot_points_node_1 + num_plot_points_node_2) : (
        num_plot_points_node_1 + num_plot_points_node_2 + num_plot_points_node_interface
    ),
    node_ID_to_plot_interface_index,
]
...
# ax.plot3D(plotting_points_coor_node_2[:,0], plotting_points_coor_node_2[:,1], shape_func_value_node_2)
ax.plot_trisurf(
    plotting_points_coor_node_1[:, 0],
    plotting_points_coor_node_1[:, 1],
    kernal_func_node_1,
    color="white",
    edgecolors="grey",
    alpha=0.5,
...
# ax.scatter(plotting_points_coor_node_1[:,0], plotting_points_coor_node_1[:,1], shape_func_value_node_1, c='red')
ax.plot_trisurf(
    plotting_points_coor_node_2[:, 0],
    plotting_points_coor_node_2[:, 1],
    kernal_func_node_2,
    color="white",
    edgecolors="grey",
    alpha=0.5,
...
ax.plot_trisurf(
    plotting_points_coor_node_interface[:, 0],
    plotting_points_coor_node_interface[:, 1],
    kernal_func_node_interface,
    color="white",
    edgecolors="grey",
    alpha=0.5,
)
...
shape_func_value, grad_shape_func_x_value, grad_shape_func_y_value = (
...
shape_func_value_node_1 = shape_func_array[
    0:num_plot_points_node_1, node_ID_to_plot_1_index
]
shape_func_value_node_2 = shape_func_array[
    num_plot_points_node_1 : (num_plot_points_node_1 + num_plot_points_node_2),
    node_ID_to_plot_2_index,
]
shape_func_value_node_interface = shape_func_array[
    (num_plot_points_node_1 + num_plot_points_node_2) : (
        num_plot_points_node_1 + num_plot_points_node_2 + num_plot_points_node_interface
    ),
    node_ID_to_plot_interface_index,
...
grad_shape_func_x_node_1 = grad_shape_func_x_array[
    0:num_plot_points_node_1, node_ID_to_plot_1_index
]
grad_shape_func_y_node_1 = grad_shape_func_y_array[
    0:num_plot_points_node_1, node_ID_to_plot_1_index
]
grad_shape_func_x_node_2 = grad_shape_func_x_array[
    num_plot_points_node_1 : (num_plot_points_node_1 + num_plot_points_node_2),
    node_ID_to_plot_2_index,
]
grad_shape_func_y_node_2 = grad_shape_func_y_array[
    num_plot_points_node_1 : (num_plot_points_node_1 + num_plot_points_node_2),
    node_ID_to_plot_2_index,
]
grad_shape_func_x_node_interface = grad_shape_func_x_array[
    (num_plot_points_node_1 + num_plot_points_node_2) : (
        num_plot_points_node_1 + num_plot_points_node_2 + num_plot_points_node_interface
    ),
    node_ID_to_plot_interface_index,
]
grad_shape_func_y_node_interface = grad_shape_func_y_array[
    (num_plot_points_node_1 + num_plot_points_node_2) : (
        num_plot_points_node_1 + num_plot_points_node_2 + num_plot_points_node_interface
    ),
    node_ID_to_plot_interface_index,
...
ax.plot_trisurf(
    plotting_points_coor_node_interface[:, 0],
    plotting_points_coor_node_interface[:, 1],
    shape_func_value_node_interface,
    color="white",
    edgecolors="grey",
    alpha=0.5,
)
...
ax.plot_trisurf(
    plotting_points_coor_node_interface[:, 0],
    plotting_points_coor_node_interface[:, 1],
    grad_shape_func_x_node_interface,
    color="white",
    edgecolors="grey",
    alpha=0.5,
)
...
ax.plot_trisurf(
    plotting_points_coor_node_interface[:, 0],
    plotting_points_coor_node_interface[:, 1],
    grad_shape_func_y_node_interface,
    color="white",
    edgecolors="grey",
    alpha=0.5,
)
