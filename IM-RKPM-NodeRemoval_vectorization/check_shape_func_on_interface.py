...
print(
    "grad shape function grad x", np.max(grad_shape_func_x), np.min(grad_shape_func_x)
)
print(
    "grad shape function grad y", np.max(grad_shape_func_y), np.min(grad_shape_func_y)
)
...
print(
    "grad_shapefunc_x_shape",
    np.shape(grad_shape_func_x),
    np.shape(grad_shape_func_x_kristen),
)
print(
    "grad_shapefunc_y_shape",
    np.shape(grad_shape_func_y),
    np.shape(grad_shape_func_y_kristen),
)
...
def allign_x_nodes_interface(
    num_nodes_interface,
    num_nodes,
    x_nodes_interface,
    x_nodes,
    x_nodes_interface_kristen,
    x_nodes_kristen,
    shape_func,
    shape_func_kristen,
    grad_shape_func_x,
    grad_shape_func_x_kristen,
    grad_shape_func_y,
    ...
    non_zero_grad_shape_x_Kristen = np.shape(
        np.where(grad_shape_func_x_kristen != 0)[0]
    )
    non_zero_grad_shape_y_Kristen = np.shape(
        np.where(grad_shape_func_y_kristen != 0)[0]
    )
    ...
    print(
        "num of nonzero grad shap func x",
        non_zero_grad_shape_x_Xin,
        non_zero_grad_shape_x_Kristen,
    )
    print(
        "num of nonzero grad shap func y",
        non_zero_grad_shape_y_Xin,
        non_zero_grad_shape_y_Kristen,
    )

    for i in range(num_nodes_interface):
        for j in range(num_nodes_interface):
            if (
                abs(x_nodes_interface[i, 0] - x_nodes_interface_kristen[j, 0]) < 1e-13
                and abs(x_nodes_interface[i, 1] - x_nodes_interface_kristen[j, 1])
                < 1e-13
            ):
                alligned_x_nodes_interface_kristen.append(
                    [x_nodes_interface[i, 0], x_nodes_interface[i, 1]]
                )
    ...
    for m in range(num_nodes):
        for n in range(num_nodes):
            if (
                abs(x_nodes[m, 0] - x_nodes_kristen[n, 0]) < 1e-13
                and abs(x_nodes[m, 1] - x_nodes_kristen[n, 1]) < 1e-13
            ):
                alligned_x_nodes_kristen.append([x_nodes[m, 0], x_nodes[m, 1]])

                alligned_shape_func[:, m] = inter_alligned_shape_func[:, n]
                alligned_grad_shape_func_x[:, m] = inter_alligned_grad_shape_func_x[
                    :, n
                ]
                alligned_grad_shape_func_y[:, m] = inter_alligned_grad_shape_func_y[
                    :, n
                ]
    ...
    for i in range(num_nodes_interface):
        for j in range(num_nodes):
            ...
            if (
                abs(alligned_grad_shape_func_x[i][j] - grad_shape_func_x[i][j])
                > 1.0e-15
            ):
                aa_grad_shape_x += 1
                mismatched_index_grad_shape_x.append([i, j])
                mismatch_grad_x.append(
                    abs(alligned_grad_shape_func_x[i][j] - grad_shape_func_x[i][j])
                )
            if (
                abs(alligned_grad_shape_func_y[i][j] - grad_shape_func_y[i][j])
                > 1.0e-15
            ):
                aa_grad_shape_y += 1
                mismatched_index_grad_shape_y.append([i, j])
                mismatch_grad_y.append(
                    abs(alligned_grad_shape_func_y[i][j] - grad_shape_func_y[i][j])
                )
                # break
    ...
    return (
        ...
        mismatched_index_grad_shape_y,
        alligned_shape_func,
        alligned_grad_shape_func_x,
        alligned_grad_shape_func_y,
    )
...
# # plt.scatter(x_G[:, 0], x_G[:, 1], c=np.linalg.norm(x_G-np.asarray(alligned_x_G_kristen),2,axis=1))
plt.scatter(
    x_nodes_interface[np.array(mismatched_index_grad_shape_x)[:, 0], 0],
    x_nodes_interface[np.array(mismatched_index_grad_shape_x)[:, 0], 1],
    c=mismatch_grad_x,
)
# plt.colorbar()
# plt.title('log(difference of x_G)')
...
plt.figure()
plt.scatter(
    x_nodes_interface[np.array(mismatched_index_grad_shape_y)[:, 1], 0],
    x_nodes_interface[np.array(mismatched_index_grad_shape_y)[:, 1], 1],
    c=mismatch_grad_y,
)
plt.colorbar()
