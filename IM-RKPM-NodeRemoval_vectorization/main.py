...
if single_grain == "True":
    ...
    )  # interface nodes has repeated nodes on interface, while x_nodes_interface_unique is unique.
...
M = np.array(
    [np.zeros((3, 3)) for _ in range(num_gauss_points_in_domain)], dtype=np.float64
)
M_P_x = np.array(
    [np.zeros((3, 3)) for _ in range(num_gauss_points_in_domain)], dtype=np.float64
)  # partial M partial x
...
shape_func_times_det_J_time_weight = csr_array(
    (
...
shape_func_weak_dis_times_det_J_time_weight_array = (
    shape_func_times_det_J_time_weight.toarray()
)
...
M_int = np.array(
    [np.zeros((3, 3)) for _ in range(num_interface_nodes)], dtype=np.float64
)
M_int_P_x = np.array(
    [np.zeros((3, 3)) for _ in range(num_interface_nodes)], dtype=np.float64
)  # partial M partial x
...
(
    phi_nonzero_index_row_int,
    phi_nonzero_index_column_int,
    phi_nonzerovalue_data_int,
    phi_P_x_nonzerovalue_data_int,
    phi_P_y_nonzerovalue_data_int,
    M_int,
    M_int_P_x,
    M_int_P_y,
...
(
    shape_func_value_int,
    shape_func_times_det_J_time_weight_value_int,
    grad_shape_func_x_value_int,
    grad_shape_func_y_value_int,
    grad_shape_func_x_times_det_J_time_weight_value_int,
    grad_shape_func_y_times_det_J_time_weight_value_int,
) = shape_grad_shape_func_int(
    x_nodes_interface_unique,
    x_nodes,
    num_non_zero_phi_a_int,
    HT0,
    M_int,
    M_int_P_x,
    M_int_P_y,
...
shape_func_times_det_J_time_weight_int = csr_array(
    (
...
grad_shape_func_x_int = csr_array(
    (
...
grad_shape_func_y_int = csr_array(
    (
...
grad_shape_func_x_times_det_J_time_weight_int = csr_array(
    (
...
grad_shape_func_y_times_det_J_time_weight_int = csr_array(
    (
...
np.savetxt(
    "shape_func_int.txt", shape_func_int.toarray()
)  # i row correspomndes to ith nodes on interface
...
comp_shape_func_grad_shape_func_on_interface = time()
print(
    "time to compute the shape function and grad of shape function on interfaces = "
    + "%s seconds"
    % (
        comp_shape_func_grad_shape_func_on_interface
...
(
    ...
    save_distance_function_dx_b,
    save_distance_function_dy_b,
    save_point_D_coor_b,
    save_heavyside_b,
    save_heavyside_px_b,
    save_heavyside_py_b,
...
if Node_removal == "True":
    all_damaged_interface_nodes_id = np.array([]).astype(
        int
    )  # including all damaged interface nodes across all time steps
    damaged_interface_nodes_id = np.array([]).astype(
        int
    )  # including damaged interface nodes from current time step
...
for ii in range(nt):
    ...
    if Node_removal == "True":
        ...
        if np.shape(damaged_interface_nodes_id)[0] != 0:
            ...
            ###################################################################
            unique_row_index_to_be_modified = np.unique(
                np.where(shape_func.toarray()[:, damaged_interface_nodes_id] != 0)[0]
            )
            shape_func_row_index_to_be_modified = unique_row_index_to_be_modified[
                np.where(shape_func.toarray()[unique_row_index_to_be_modified, :] != 0)[
                    0
                ]
            ]
            shape_func_column_index_to_be_modified = np.where(
                shape_func.toarray()[unique_row_index_to_be_modified, :] != 0
            )[1]

            num_shape_func_tobe_modified = np.shape(
                shape_func_column_index_to_be_modified
            )[0]

            if num_shape_func_tobe_modified != 0:

                M_modi = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified)],
                    dtype=np.float64,
                )
                M_modi_P_x = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified)],
                ...
                (
                    ...
                    grad_shape_func_x_array,
                    grad_shape_func_y_array,
                    ...
                    phi_x.toarray(),
                    phi_y.toarray(),
                    shape_func_row_index_to_be_modified,
                    shape_func_column_index_to_be_modified,
                    HT0,
                    HT1,
                    HT2,
                    ...
                    shape_func.toarray(),
                    shape_func_times_det_J_time_weight.toarray(),
                    grad_shape_func_x.toarray(),
                    grad_shape_func_y.toarray(),
                    grad_shape_func_x_times_det_J_time_weight.toarray(),
                    grad_shape_func_y_times_det_J_time_weight.toarray(),
                    damaged_interface_nodes_id,
                )

                phi = csr_array(phi_array)
                phi_x = csr_array(phi_x_array)
                phi_y = csr_array(phi_y_array)

                shape_func = csr_array(shape_func_array)
                shape_func_times_det_J_time_weight = csr_array(
                    shape_func_times_det_J_time_weight_array
                )
                grad_shape_func_x = csr_array(grad_shape_func_x_array)
                grad_shape_func_y = csr_array(grad_shape_func_y_array)
                grad_shape_func_x_times_det_J_time_weight = csr_array(
                    grad_shape_func_x_times_det_J_time_weight_array
                )
                grad_shape_func_y_times_det_J_time_weight = csr_array(
            ...
            ###################################################################
            unique_row_index_to_be_modified_b = np.unique(
                np.where(shape_func_b.toarray()[:, damaged_interface_nodes_id] != 0)[0]
            )
            shape_func_row_index_to_be_modified_b = unique_row_index_to_be_modified_b[
                np.where(
                    shape_func_b.toarray()[unique_row_index_to_be_modified_b, :] != 0
                )[0]
            ]
            shape_func_column_index_to_be_modified_b = np.where(
                shape_func_b.toarray()[unique_row_index_to_be_modified_b, :] != 0
            )[1]

            num_shape_func_tobe_modified_b = np.shape(
                shape_func_column_index_to_be_modified_b
            )[0]

            if num_shape_func_tobe_modified_b != 0:
                M_modi_b = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_b)],
                    dtype=np.float64,
                )
                M_modi_P_x_b = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_b)],
                    dtype=np.float64,
                )  # partial M partial x
                M_modi_P_y_b = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_b)],
                    dtype=np.float64,
                )  # partial M partial y
                ...
                (
                    ...
                    grad_shape_func_b_x_array,
                    grad_shape_func_b_y_array,
                    ...
                    phi_x_b.toarray(),
                    phi_y_b.toarray(),
                    shape_func_row_index_to_be_modified_b,
                    shape_func_column_index_to_be_modified_b,
                    HT0,
                    HT1,
                    HT2,
                    ...
                    IM_RKPM,
                    det_J_b_time_weight,
                    shape_func_b.toarray(),
                    shape_func_b_times_det_J_b_time_weight.toarray(),
                    grad_shape_func_b_x.toarray(),
                    grad_shape_func_b_y.toarray(),
                    grad_shape_func_b_x_times_det_J_b_time_weight.toarray(),
                    grad_shape_func_b_y_times_det_J_b_time_weight.toarray(),
                    damaged_interface_nodes_id,
                )

                phi_b = csr_array(phi_b_array)
                phi_x_b = csr_array(phi_x_b_array)
                phi_y_b = csr_array(phi_y_b_array)

                shape_func_b = csr_array(shape_func_b_array)
                shape_func_b_times_det_J_b_time_weight = csr_array(
                    shape_func_b_times_det_J_b_time_weight_array
                )
                grad_shape_func_b_x = csr_array(grad_shape_func_b_x_array)
                grad_shape_func_b_y = csr_array(grad_shape_func_b_y_array)
                grad_shape_func_b_x_times_det_J_b_time_weight = csr_array(
                    grad_shape_func_b_x_times_det_J_b_time_weight_array
                )
                grad_shape_func_b_y_times_det_J_b_time_weight = csr_array(
            ...
            ###################################################################
            unique_row_index_to_be_modified_int = np.unique(
                np.where(shape_func_int.toarray()[:, damaged_interface_nodes_id] != 0)[
                    0
                ]
            )
            shape_func_row_index_to_be_modified_int = (
                unique_row_index_to_be_modified_int[
                    np.where(
                        shape_func_int.toarray()[unique_row_index_to_be_modified_int, :]
                        != 0
                    )[0]
                ]
            )
            shape_func_column_index_to_be_modified_int = np.where(
                shape_func_int.toarray()[unique_row_index_to_be_modified_int, :] != 0
            )[1]

            num_shape_func_tobe_modified_int = np.shape(
                shape_func_column_index_to_be_modified_int
            )[0]

            if num_shape_func_tobe_modified_int != 0:

                M_modi_int = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_int)],
                    dtype=np.float64,
                )
                M_modi_P_x_int = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_int)],
                    dtype=np.float64,
                )  # partial M partial x
                M_modi_P_y_int = np.array(
                    [np.zeros((3, 3)) for _ in range(num_shape_func_tobe_modified_int)],
                    dtype=np.float64,
                )  # partial M partial y
                ...
                (
                    ...
                    phi_x_int_array,
                    phi_y_int_array,
                    shape_func_int_array,
                    shape_func_times_det_J_time_weight_int_array,
                    grad_shape_func_x_int_array,
                    grad_shape_func_y_int_array,
                    ...
                    IM_RKPM,
                    det_J_time_weight,
                    shape_func_int.toarray(),
                    shape_func_times_det_J_time_weight_int.toarray(),
                    grad_shape_func_x_int.toarray(),
                    grad_shape_func_y_int.toarray(),
                    grad_shape_func_x_times_det_J_time_weight_int.toarray(),
                    grad_shape_func_y_times_det_J_time_weight_int.toarray(),
                    damaged_interface_nodes_id,
                )

                phi_int = csr_array(phi_int_array)
                phi_x_int = csr_array(phi_x_int_array)
                phi_y_int = csr_array(phi_y_int_array)
                ...
                shape_func_int = csr_array(shape_func_int_array)
                shape_func_times_det_J_time_weight_int = csr_array(
                    shape_func_times_det_J_time_weight_int_array
                )
                grad_shape_func_x_int = csr_array(grad_shape_func_x_int_array)
                grad_shape_func_y_int = csr_array(grad_shape_func_y_int_array)
                grad_shape_func_x_times_det_J_time_weight_int = csr_array(
                    grad_shape_func_x_times_det_J_time_weight_int_array
                )
                grad_shape_func_y_times_det_J_time_weight_int = csr_array(
            ...
            c_n1 = np.dot(
                np.linalg.pinv(
                    np.dot(
                        np.transpose(shape_func.toarray()),
                        shape_func_times_det_J_time_weight.toarray(),
                    )
                ),
                np.dot(
    ...
    if Node_removal == "True" and np.shape(all_damaged_interface_nodes_id)[0] != 0:
        ...
        K_mechanical[all_damaged_interface_nodes_id, :] = 0.0
        K_mechanical[all_damaged_interface_nodes_id, all_damaged_interface_nodes_id] = (
            1.0
        )
        ...
        K_mechanical = csr_array(K_mechanical)
    ...
    !!!!! reshape (grad_shape_func_x*ux)
    """
    epsilon_x_int = (grad_shape_func_x_int @ ux).reshape(
        num_interface_nodes, 1
    )  # normal strain along x at nodes on interfaces
    epsilon_y_int = (grad_shape_func_y_int @ uy).reshape(
        num_interface_nodes, 1
    )  # normal strain along y at nodes on interfaces
    gamma_xy_int = (
        (grad_shape_func_x_int @ uy + grad_shape_func_y_int @ ux) * 0.5
    ).reshape(
        num_interface_nodes, 1
    ...
    beta_1_int = (
        delta_a_lattice_int / a_lattice_ini_int
    ...
    beta_2_int = (
        delta_c_lattice_int / c_lattice_ini_int
    ...
    # calculate the principle strain:
    epsilon_1_int = (epsilon_x_mechanical_int + epsilon_y_mechanical_int) / 2 + (
        ((epsilon_x_mechanical_int - epsilon_y_mechanical_int) / 2) ** 2
        + gamma_xy_mechanical_int**2
    ) ** 0.5
    epsilon_2_int = (epsilon_x_mechanical_int + epsilon_y_mechanical_int) / 2 - (
        ((epsilon_x_mechanical_int - epsilon_y_mechanical_int) / 2) ** 2
        + gamma_xy_mechanical_int**2
    ) ** 0.5
    ...
    if Node_removal == "True":
        ...
        damaged_interface_nodes_id = np.array(x_nodes_interface_unique_id)[
            np.where(D_damage_int == 1.0)[0].astype(int)
        ]

        # new damaged nodes
        damaged_interface_nodes_id = np.array(
            [
                element
                for element in damaged_interface_nodes_id
                if element not in all_damaged_interface_nodes_id
            ]
        )
        # add new damaged nodes id to all damaged node id array
        all_damaged_interface_nodes_id = np.append(
            all_damaged_interface_nodes_id, damaged_interface_nodes_id
        ).astype(int)

        # save all damaged interface node id to array
        all_damaged_interface_nodes_array[
            ii, 0 : np.shape(all_damaged_interface_nodes_id)[0]
        ] = all_damaged_interface_nodes_id
    ...
    if ii == 0:
        comp_solve_mechanical_stress = time()
        print(
...
np.savetxt(
    "all_damaged_interface_nodes_history229.txt", all_damaged_interface_nodes_array
)
