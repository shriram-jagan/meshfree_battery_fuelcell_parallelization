...
def i_0_complex(x):
    ...
    A2 = 4.420894220185683e02
    A3 = -5.783762746199664e03
    A4 = 3.822682327855755e04
    A5 = -1.416477460103355e05
    A6 = 3.113802647858406e05
    A7 = -4.169011915077865e05
    A8 = 3.347705415406199e05
    A9 = -1.485221335897379e05
    A10 = 2.803425068966447e04
    P = (
        (
            (
                ((((((A10 * x + A9) * x + A8) * x + A7) * x + A6) * x + A5) * x + A4)
                * x
                + A3
            )
            * x
...
def Dn_complex(x, D_damage):
    ...
    D_coefs = [
        ...
        0.00000000000000000000,
        0.00000000000000000000,
        0.00000000000001231320,
        0.00000000000000000000,
        0.00000000000000000000,
        0.00000000000000000000,
        0.00000000000001231320,
        -0.00000000000000006780,
        -0.00000000000000041949,
        0.00000000000000000000,
        0.00000000000001231320,
        0.00000000000000880302,
        -0.00000000000001638013,
        -0.00000000000000027000,
        0.00000000000001227362,
        0.00000000000125685839,
        -0.00000000000037054532,
        -0.00000000000000576569,
        0.00000000000001163483,
        -0.00000000000000036183,
        0.00000000000000543084,
        -0.00000000000000316081,
        0.00000000000000571475,
    ]
    ...
    for ii in range(6):
        ...
        dD_dx = dD_dx + logic_x * (
            D_dpolydx_coefs[ii, 0] * (x - D_x_thresholds[ii]) ** 2
            + D_dpolydx_coefs[ii, 1] * (x - D_x_thresholds[ii]) ** 1
            + D_dpolydx_coefs[ii, 2] * (x - D_x_thresholds[ii]) ** 0
        )

    return (
        D * macro_to_grain * (1 - D_damage),
        dD_dx * macro_to_grain * (1 - D_damage),
    )
...
def ocp_complex(x):
    ...
    for ii in range(18):
        ...
        dEeq_dx = dEeq_dx + logic_x * (
            Eeq_dpolydx_coefs[ii, 0] * (x - Eeq_x_thresholds[ii]) ** 2
            + Eeq_dpolydx_coefs[ii, 1] * (x - Eeq_x_thresholds[ii]) ** 1
            + Eeq_dpolydx_coefs[ii, 2] * (x - Eeq_x_thresholds[ii]) ** 0
        )
