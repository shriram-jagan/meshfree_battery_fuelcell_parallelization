...
@jit
def modify_shape_func_node_removal(
    itt,
    x_nodes,
    x_G,
    M_modi,
    ...
    for i_m in range(np.shape(shape_func_row_index_to_be_modified)[0]):
        ...
        for j in range(np.shape(x_nodes)[0]):
            ...
            for ii in range(3):
                for jj in range(3):
                    ...
                    M_modi[i_m][ii][jj] = (
                        M_modi[i_m][ii][jj] + H[ii] * H_T[jj] * phi_scaled[i, j]
                    )
                    M_modi_P_x[i_m][ii][jj] = (
                        M_modi_P_x[i_m][ii][jj]
                        + H[ii] * H_T[jj] * phi_x_scaled[i, j]
                        + H_P_x[ii] * H_T[jj] * phi_scaled[i, j]
                        + H[ii] * HT_P_x[jj] * phi_scaled[i, j]
                    )
                    M_modi_P_y[i_m][ii][jj] = (
                        M_modi_P_y[i_m][ii][jj]
                        + H[ii] * H_T[jj] * phi_y_scaled[i, j]
                        + H_P_y[ii] * H_T[jj] * phi_scaled[i, j]
                        + H[ii] * HT_P_y[jj] * phi_scaled[i, j]
                    )
            ...
            np.dot(
                (
                    np.dot(
                        ...
                        (np.linalg.inv(M_modi[i_m])).astype(np.float64),
                    )
                ).astype(np.float64),
                H.astype(np.float64),
            )
        ...
        if differential_method == "implicite" and IM_RKPM == "False":
            ...
            if differential_method == "direct" or IM_RKPM == "True":
                M_inv_P_x_i = -np.dot(
                    np.dot(
                        np.linalg.inv(M_modi[i_m].astype(np.float64)).astype(
                            np.float64
                        ),
                        M_modi_P_x[i_m].astype(np.float64),
                    ),
                    np.linalg.inv(M_modi[i_m].astype(np.float64)).astype(np.float64),
                )
                M_inv_P_y_i = -np.dot(
                    np.dot(
                        np.linalg.inv(M_modi[i_m].astype(np.float64)).astype(
                        ...
                        M_modi_P_y[i_m].astype(np.float64),
                    ),
                    np.linalg.inv(M_modi[i_m].astype(np.float64)).astype(np.float64),
                )
                grad_shape_func_x_ij = (
        ...
        grad_shape_func_x_times_det_J_time_weight[i, j] = (
            grad_shape_func_x_ij * det_J_time_weight[i]
        )
        grad_shape_func_y_times_det_J_time_weight[i, j] = (
