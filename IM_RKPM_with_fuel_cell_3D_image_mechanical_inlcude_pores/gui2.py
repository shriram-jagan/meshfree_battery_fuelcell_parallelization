...
class Application(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.categories = {
            ...
            "Execution": [{"label": "Number of Cores", "entry": None}],
        }
            ...
            for line in file:
                ...
                if line.endswith(":"):
                    ...
                    key, value = line.split(
                        ":", 1
                    )  # return list of string, the string is seperated by :
    ...
    def create_frames(self):
        ...
        for (
            ...
            self.categories
        ):  # category is the most upper level key, category of each frame, geometry, time....
            self.frames[category] = self.create_category_frame(
                category, self.categories[category]
            )

    def create_category_frame(
        self, category_name, entries
    ):  # entries is al variables under a category, it is a list, the component of this list is library
        frame = tk.Frame(self, relief="groove", borderwidth=2)  # create frame
        frame.focus_set()
        title_label = tk.Label(
            frame, text=category_name, font=("Helvetica", 20, "bold")
        )
        title_label.place(
            relx=0.6, rely=0.1, anchor=tk.CENTER
        )  # category name in a single line
        if category_name == "Introduction":
            ...
            preset_values = self.preset_values.get(
                category_name, {}
            )  # library:{'xmin': value, 'xmax': value2.....}
            for i, entry in enumerate(entries):
                ...
                var_label.place(
                    relx=0.3, rely=0.2 + i / 10, anchor=tk.CENTER
                )  # label name
                if entry["label"] == "single_grain":
                    ...
                    options = ["True", "Faulse"]
                    entry["entry"] = tk.OptionMenu(
                        frame, self.selected_option_single_grain, *options
                    )
                    ...
                    if entry["label"] == "differential_method":
                        ...
                        entry["entry"] = tk.OptionMenu(
                            frame, self.selected_option_differential_method, *options
                        )
                        entry["entry"].place(
                            relx=0.6, rely=0.2 + i / 10, anchor=tk.CENTER
                        )
                        ...
                        if entry["label"] == "integral_method":
                            ...
                            options = ["gauss", "SCNI"]
                            entry["entry"] = tk.OptionMenu(
                                frame, self.selected_option_integral_method, *options
                            )
                            entry["entry"].place(
                                relx=0.6, rely=0.2 + i / 10, anchor=tk.CENTER
                            )
        ...
        categorybotton_intro = tk.Button(
            frame,
            text="Introduction",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Introduction"),
            background="blue",
        )
        categorybotton_intro.place(
            relx=0.1, rely=0.2, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_geo = tk.Button(
            frame,
            text="Geometry",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Geometry"),
            background="blue",
        )
        categorybotton_geo.place(
            relx=0.1, rely=0.3, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_time = tk.Button(
            frame,
            text="Time Step Setup",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Time Step Setup"),
            background="blue",
        )
        categorybotton_time.place(
            relx=0.1, rely=0.4, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_mshfreedom = tk.Button(
            frame,
            text="Mesh Free Method",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Mesh Free Method"),
            background="blue",
        )
        categorybotton_mshfreedom.place(
            relx=0.1, rely=0.5, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_diffu = tk.Button(
            frame,
            text="Diffusion",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Diffusion"),
            background="blue",
        )
        categorybotton_diffu.place(
            relx=0.1, rely=0.6, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_mech = tk.Button(
            frame,
            text="Mechanical",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Mechanical"),
            background="blue",
        )
        categorybotton_mech.place(
            relx=0.1, rely=0.7, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
        categorybotton_ex = tk.Button(
            frame,
            text="Execution",
            font=("Helvetica", 12, "bold"),
            command=lambda: self.show_frame("Execution"),
            background="blue",
        )
        categorybotton_ex.place(
            relx=0.1, rely=0.8, anchor=tk.CENTER, relwidth=0.2, relheight=0.1
        )
    ...
    def show_frame(self, category):
        ...
        )  # show the next or previous frame
                    ...
                    for entry in entries:
                        if entry["entry"]:
                            if entry["entry"].get() != "":
                                ...
                                file.write(
                                    f"  {entry['label']}: {entry['entry'].get()}\n"
                                )  # get from entry of gui

                        else:
                            if entry["label"] == "differential_method":
                                file.write(
                                    f"  {entry['label']}: {self.selected_option_differential_method.get()}\n"
                                )  # get from entry of gui
                            if entry["label"] == "integral_method":
                                file.write(
                                    f"  {entry['label']}: {self.selected_option_integral_method.get()}\n"
                                )  # get from entry of gui
                            if entry["label"] == "single_grain":
                                file.write(
                                    f"  {entry['label']}: {self.selected_option_single_grain.get()}\n"
                                )  # get from entry of gui
